...
enum WgIdent { case none,resolution,morph,rotate,stereo,showBalls,doInversion,fourGen,saveLoad,reset,controlDisplay,help,texture,move,dfClamp }
...
class WidgetGroup: NSView {
        ...
        for i in 0 ..< data.count {
            if s == data[i].hotKey {
                ...
                case .command :
                    ...
                    if [ .move, .rotate ].contains(data[i].ident) { focus = i }
    ...
    func drawEntry(_ index:Int) {
        if vc.isMorph && shouldMemorizeFocus(index) && data[index].morph {
            wgMorphColor.set()
            let r = CGRect(x:1, y:py-3, width:bounds.width-2, height:CGFloat(RowHT)+2)
